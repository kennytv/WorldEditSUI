...
public final class WorldEditSUIPlugin extends JavaPlugin {
    ...
    private void updateSelections() {
        // Don't let two runs write to the caches at the same time
        if (!updatingSelections.compareAndSet(false, true)) {
            return;
        }

        try {
            for (final Player player : getServer().getOnlinePlayers()) {
                updateSelections(player);
            }
        } finally {
            updatingSelections.set(false);
        }
    }

    private void updateSelections(final Player player) {
        if (!player.isOnline()) {
            return;
        }
